...
export default function PileTooltip({ isVisible, count, label, mirrored = false }: PileTooltipProps) {
  if (count <= 0) return;
  ...
  return (
    ...
    ${mirrored ? `left-full ml-2 ${isVisible ? "translate-x-0" : "-translate-x-2"}` : `right-full mr-2 ${isVisible ? "translate-x-0" : "translate-x-2"}`}`}
    >
      {count} {label}
    </div>
  );
