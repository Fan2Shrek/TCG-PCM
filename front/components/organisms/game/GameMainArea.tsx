        ...
        <div className='h-[95vh] min-h-300 w-[90vw] min-w-450 bg-orange-800 flex flex-col pb-25 relative'>
          ...
          {oppositePlayer && (
            <div className='w-full h-full relative grid grid-cols-5 items-center gap-5 bg-red-600 p-3'>
              <DrawPile numCards={oppositePlayer.drawPile.length} className='col-span-1' />
              <div className='flex flex-col col-span-3 items-center'>
                <EnemyPlayZone title='Player 2 Cards' passiveCardIds={oppositePlayer.playArea.passiveCards} monsterCardIds={oppositePlayer.playArea.monsterCards} />
                <PlayerCharacterDisplay player={opponentState} className='absolute top-0' />
              </div>
              <Cemetery cardIds={oppositePlayer.discardPile} className='col-span-1' />
            </div>
          )}

          {loggedPlayer && (
            <div className='w-full h-full relative grid grid-cols-5 items-center gap-5 bg-blue-600 p-3'>
              <Cemetery cardIds={loggedPlayer.discardPile} className='col-span-1' />
              <div className='flex flex-col col-span-3 items-center h-full'>
                <PlayZone title='Player 1 Cards' passiveCardIds={loggedPlayer.playArea.passiveCards} monsterCardIds={loggedPlayer.playArea.monsterCards} />
                <PlayerCharacterDisplay player={currentState} className='absolute bottom-0' />
              </div>
              <DrawPile numCards={loggedPlayer.drawPile.length} className='col-span-1' />
            </div>
          )}
        </div>
      </div>
